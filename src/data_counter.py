...
def data_counter():
    ...
    qte_formacoes_iniciadas.to_csv(f'{DIR}/quantitative/contHistoricosInicio.csv', sep=',', encoding='utf-8', index=False)
    ...
    #create a csv file with the data ####################
    qte_formacoes_concluidas.to_csv(f'{DIR}/quantitative/contHistoricosFim.csv', sep=',', encoding='utf-8', index=False)

    #check if the analysis is from a specific university ####################
    if ANALISE_ESPECIFICA:
        #if it is, get the historical data from the university ####################
        historicos_na_universidade = pd.read_csv(f'{DIR}/primary/historicosUniversidade.csv', sep=',')
        ...
        #create a csv file with the data ####################
        qte_formacoes_iniciadas_na_universidade.to_csv(f'{DIR}/quantitative/qte_formacoes_iniciadas_na_universidade.csv', sep=',', encoding='utf-8', index=False)
        ...
        #create a csv file with the data ####################
        qte_formacoes_concluidas_na_universidade.to_csv(f'{DIR}/quantitative/qte_formacoes_concluidas_na_universidade.csv', sep=',', encoding='utf-8', index=False)
    ...
    qte_local_de_nascimento.to_csv(f'{DIR}/quantitative/contNascimento.csv', sep=',', encoding='utf-8')
    ...
    qte_local_de_atuacao.to_csv(f'{DIR}/quantitative/contAtuacao.csv', sep=',', encoding='utf-8')

    #get the fields of research ####################
    areas_de_atuacao = pd.read_csv(f'{DIR}/primary/areasDeAtuacao.csv', sep=',')

    #count the fields of research ####################
    qte_areas_de_atuacao = areas_de_atuacao.value_counts(['Grande Área','Área']).to_frame('Quantidade')
    qte_areas_de_atuacao.to_csv(f'{DIR}/quantitative/areas_de_atuacao.csv', sep=',', encoding='utf-8')
